...
var Pagination = {
  ...
  Click: function Click() {
    ...
    var glempAllPagin = Pagination.globj.slice((Pagination.page - 1) * Pagination.perPage, Pagination.perPage * Pagination.page);
  ...
  Create: function Create(e) {
    var icon_prev = "<svg width=\"10\" height=\"14\" viewBox=\"0 0 10 14\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">\n        <path d=\"M9 1L2 7L9 13\" stroke=\"#5E6D77\" stroke-width=\"1.5\"/>\n        </svg>";
    var icon_next = "<svg width=\"10\" height=\"14\" viewBox=\"0 0 10 14\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">\n        <path d=\"M1 13L8 7L1 1\" stroke=\"#5E6D77\" stroke-width=\"1.5\"/>\n        </svg>";
    var html = ['<a class="prev">' + icon_prev + '</a>',
    // previous button
...
var glempsAll = JSON.parse(glamping_club_ajax.glAll);
var glIt = document.querySelector('.glampings-items');
if (glIt) {
  if (glIt.classList.contains('no-map')) {
    if (window.innerWidth > 1649) {
      var perPageFullSt = Number(glamping_club_ajax.glc_per_page_full);
      var glempAllPagin = glempsAll.slice(0, perPageFullSt);
      glempRender(glempAllPagin);
      var pagesAbs = glempsAll.length / perPageFullSt;
      var pages = Math.ceil(pagesAbs);
      var navLinks = document.querySelector('.filtr-pagination .nav-links');
      if (pages > 1) {
  ...
  if (document.querySelector('.glampings-filtr-btns__item.btn-clear')) {
    if (!document.querySelector('.glampings-filtr-btns__item.btn-clear').classList.contains('active')) {
      if (glIt) {
        if (glIt.classList.contains('no-map')) {
          if (window.innerWidth > 1649) {
            var _perPageFullSt = Number(glamping_club_ajax.glc_per_page_full);
            var _glempAllPagin = glempsAll.slice(0, _perPageFullSt);
            glempRender(_glempAllPagin);
            var _pagesAbs = glempsAll.length / _perPageFullSt;
            var _pages = Math.ceil(_pagesAbs);
            var _navLinks = document.querySelector('.filtr-pagination .nav-links');
            if (_pages > 1) {
              if (document.querySelector('.pagination')) {
                document.querySelector('.pagination').style.display = 'none';
              }
            ...
            Pagination.Init(_navLinks, {
              ...
              // pages before and after current
              globj: glempsAll,
              perPage: _perPageFullSt
            });
          } else {
            var perPageAllSt = Number(glamping_club_ajax.glc_per_page_all);
            var _glempAllPagin2 = glempsAll.slice(0, perPageAllSt);
            glempRender(_glempAllPagin2);
            var _pagesAbs2 = glempsAll.length / perPageAllSt;
            var _pages2 = Math.ceil(_pagesAbs2);
            var _navLinks2 = document.querySelector('.filtr-pagination .nav-links');
            if (_pages2 > 1) {
              if (document.querySelector('.pagination')) {
                document.querySelector('.pagination').style.display = 'none';
              }
...
var agc = function agc(glAll) {
  var allGlCount = document.querySelectorAll('.all-gl-count');
  if (!allGlCount.length) return;
  allGlCount.forEach(function (item, i) {
    item.innerText = glAll.length;
  });
};
agc(glempsAll);
if (localStorage.getItem('glcRefFront') == 'front') {
  var glAll = JSON.parse(glamping_club_ajax.glAll);
  var glampingsMap = document.querySelector('.glampings-map');
  var locVal = localStorage.getItem('glcRegion');
  var typeVal = localStorage.getItem('glcType');
  if (locVal && typeVal) {
    console.dir(glAll.filter(function (gl) {
      return gl.location.includes(locVal);
    }).filter(function (gl) {
      return gl.type.includes(typeVal);
    }));
    glempsAlls = glAll.filter(function (gl) {
      return gl.location.includes(locVal);
    }).filter(function (gl) {
      return gl.type.includes(typeVal);
    });
...
var filtrStocks = function filtrStocks(glempAll) {
  var stocksItem = document.querySelector('.filtr-item.stocks');
  if (!stocksItem) return;
  var stocksItemCount = document.querySelector('.filtr-item.stocks span.count');
  var stocksCount = 0;
...
var filtrRecommended = function filtrRecommended(glempAll) {
  var recommendedItem = document.querySelector('.filtr-item.recommended');
  if (!recommendedItem) return;
  var recommendedItemCount = document.querySelector('.filtr-item.recommended span.count');
...
function sliderNumber(startMin, startMax, min, max) {
  ...
  slider.noUiSlider.on('update', function () {
    var numbFmt = new Intl.NumberFormat('ru-RU');
  ...
  slider.noUiSlider.on('end', function (values, handle) {
    ...
    var newgGlempAll = glempsAll.filter(filtrOptionsChange).filter(priceRange, sliderValue);
    ...
    var names = ['glcType', 'glcAllocation', 'glcWorking', 'glcNature', 'glcFacilGen', 'glcFacilChildren', 'glcEntertainment', 'glcTerritory', 'glcSafety', 'glcStocks', 'glcRecom'];
...
function itemsChange() {
  ...
  filtrItems.addEventListener('click', function (event) {
    ...
    var glcStocks = localStorage.getItem('glcStocks');
    var glcRecom = localStorage.getItem('glcRecom');
    var names = ['glcType', 'glcAllocation', 'glcWorking', 'glcNature', 'glcFacilGen', 'glcFacilChildren', 'glcEntertainment', 'glcTerritory', 'glcSafety', 'glcStocks', 'glcRecom'];
    if (input) {
      var glempAll = JSON.parse(glamping_club_ajax.glAll);
      var newgGlempAll = glempAll.filter(filtrOptionsChange);
      var newgGlempAllForFiltr = glempAll.filter(filtrOptionsChange);
      ...
      if (document.querySelector('input[name="stocks"]').checked) {
        newgGlempAll = glempAll.filter(filtrOptionsChange).filter(priceRange, priceObj.map(Number)).filter(function (elem) {
          return elem.stocks != '';
        });
        newgGlempAllForFiltr = glempAll.filter(filtrOptionsChangeForFiltr).filter(priceRange, priceObj.map(Number)).filter(function (elem) {
          return elem.stocks != '';
        });
      }
      if (document.querySelector('input[name="recommended"]').checked) {
        newgGlempAll = glempAll.filter(filtrOptionsChange).filter(priceRange, priceObj.map(Number)).filter(function (elem) {
          return elem.recommended == 'yes';
        });
        newgGlempAllForFiltr = glempAll.filter(filtrOptionsChangeForFiltr).filter(priceRange, priceObj.map(Number)).filter(function (elem) {
          return elem.recommended == 'yes';
        });
      ...
      if (sortGl) {
        if (sortGl == 'new_items') {
          newgGlempAll.sort(function (x, y) {
      ...
      var perPageAll = Number(glamping_club_ajax.glc_per_page_all);
      var perPageFull = Number(glamping_club_ajax.glc_per_page_full);
      var perPage = perPageAll;
      if (window.innerWidth > 1649) {
        perPage = perPageFull;
      }
      var _glempAllPagin3 = newgGlempAll.slice(0, perPage);
      glempRender(_glempAllPagin3);
      var _pagesAbs3 = newgGlempAll.length / perPage;
      var _pages3 = Math.ceil(_pagesAbs3);
      var _navLinks3 = document.querySelector('.filtr-pagination .nav-links');
      if (_pages3 > 1) {
        if (document.querySelector('.pagination')) {
          document.querySelector('.pagination').style.display = 'none';
        }
      ...
      window.addEventListener('resize', function () {
        if (document.querySelector('.glampings-items').classList.contains('no-map')) {
          if (window.innerWidth > 1649) {
            var _perPageFullSt2 = Number(glamping_club_ajax.glc_per_page_full);
            var _glempAllPagin4 = newgGlempAll.slice(0, _perPageFullSt2);
            glempRender(_glempAllPagin4);
            var _pagesAbs4 = newgGlempAll.length / _perPageFullSt2;
            var _pages4 = Math.ceil(_pagesAbs4);
            var _navLinks4 = document.querySelector('.filtr-pagination .nav-links');
            if (_pages4 > 1) {
              if (document.querySelector('.pagination')) {
                document.querySelector('.pagination').style.display = 'none';
              }
            ...
            Pagination.Init(_navLinks4, {
              ...
              // pages before and after current
              globj: newgGlempAll,
              perPage: _perPageFullSt2
            });
          } else {
            var perPageAllSt = Number(glamping_club_ajax.glc_per_page_all);
            var _glempAllPagin5 = newgGlempAll.slice(0, perPageAllSt);
            glempRender(_glempAllPagin5);
            var _pagesAbs5 = newgGlempAll.length / perPageAllSt;
            var _pages5 = Math.ceil(_pagesAbs5);
            var _navLinks5 = document.querySelector('.filtr-pagination .nav-links');
            if (_pages5 > 1) {
              if (document.querySelector('.pagination')) {
                document.querySelector('.pagination').style.display = 'none';
              }
...
function checkLocalCheng(item, lsName, filtrItemCount) {
  var count = item.closest('div.filtr-item').children[0].children[1];
  var ls_obj = [];
  var ls_obj_new = [];
...
function glempRender(glemps) {
  ...
  var glcFav = favoritesRenderFiltr('glcFav');
  var glcCompar = favoritesRenderFiltr('glcCompar');
  ...
  glemps.forEach(function (glemp) {
    ...
    var stocks = glemp.stocks_title;
    var stocksRend = "";
    if (stocks) {
      stocksRend = "<div class=\"glamping-item__sr__item stocks\">".concat(glemp.stocks_title, "</div>");
    }
...
function mapRender(geoData) {
  ...
  function init() {
    ...
    map.geoObjects.events.add('balloonopen', function (e) {
      var id = e.get('objectId');
      // console.dir(id);
      var addFavorites = document.querySelector('.ymaps-2-1-79-balloon__content #add-favorites');
      var addComparison = document.querySelector('.ymaps-2-1-79-balloon__content #add-comparison');
      var supFavorites = document.querySelectorAll('#sup-favorites');
      var supComparison = document.querySelectorAll('#sup-comparison');
      var glcFav = favoritesRenderNologin('glcFav');
      var glcCompar = favoritesRenderNologin('glcCompar');
      if (addFavorites) {
        ...
        addFavorites.addEventListener('click', function (e) {
          var addFavoritesBtnSingle = document.querySelector('button#add-favorites[data-postid="' + addFavorites.dataset.postid + '"]');
      ...
      if (addComparison) {
        ...
        addComparison.addEventListener('click', function (e) {
          var addComparisonBtnSingle = document.querySelector('button#add-comparison[data-postid="' + addComparison.dataset.postid + '"]');
...
function filtrHidden(map) {
  var btns = document.querySelectorAll('.js-btn-filtr');
  btns.forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      var mainWidth = document.querySelector('.archive-glampings').clientWidth; //-690
      var glLeft = document.querySelector('.archive-glampings__left').clientWidth;
      var glMap = document.querySelector('.glampings-map').clientWidth;
      var filtr = document.querySelector('.glampings-filtr');
      var btnHidden = document.querySelector('.glampings-filtr-hidden');
      ...
      if (document.querySelector('.glampings-filtr').classList.contains('visible-hidden')) {
        ...
        var mapWidth = mainWidth - 960;
        document.querySelector('.glampings-map__content').style.width = mainWidth + 'px';
        map.container.getElement().style.width = mapWidth + 'px';
        map.container.fitToViewport();
      } else {
        // filtr.style.height = 0;
        var _mapWidth = glMap + 270;
        document.querySelector('.glampings-map__content').style.width = _mapWidth + 'px'; //elemWidth+'px';
        map.container.getElement().style.width = _mapWidth + 'px';
        map.container.fitToViewport();
      }
    });
  });
...
function mapPointTest(glAll) {
  ...
  glAll.forEach(function (item) {
    ...
    // let rating = reviews_stars_items_average( 2.9, 4 );
    var rating = reviews_stars_items_average(item.average_rating, item.count_rating);
    ...
    if (media_urls.length) {
      ...
      media_urls.forEach(function (media_url) {
        ...
        mediaUrls += "<img width=\"80\" height=\"80\" src=\"".concat(media_url, "\" class=\"attachment-map-image\" alt=\"\" decoding=\"async\">");
        mediaUrls += "</div>";
      });
      mediaUrls += "</div>";
...
var listCardMap = function listCardMap() {
  var btnMap = document.querySelectorAll('.js-btn-map button');
  if (!btnMap.length) return;
  var archiveGlampings = document.querySelector('#archive-glampings');
  var glWrap = document.querySelector('.glampings-wrap');
  ...
  var archGlampingsLeftGlamp = document.querySelector('.archive-glampings__left__glampings');
  var glcfScroll = document.querySelector('.glcf-scroll');
  btnMap.forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      btnMapChange(btnMap);
      btn.classList.add('active');
      if (btn.id == 'mapVision') {
        ...
        glampingsMap.children[0].innerHTML = '';
        var newgGlempAll = glempsAll.filter(filtrOptionsChange);
        var priceObj = [];
...
function newScrollbarGlCard() {
  ...
  var elem = document.querySelector('.my-scrollbar-gc');
  if (!elem) return;
  var sgc = Scrollbar.init(elem, {
...
function sortGlempRender(sortGl) {
  ...
  // let glempAll = JSON.parse(glamping_club_ajax.glAll);
  var newgGlempAll = glempsAll.filter(filtrOptionsChange).filter(priceRange, priceObj.map(Number));
...
var addFavCom = function addFavCom() {
  var favoritesBtns = document.querySelectorAll('#add-favorites');
  var comparisonBtns = document.querySelectorAll('#add-comparison');
  var singleGlampings = document.querySelector('.single-glampings');
  var comparesMain = document.querySelector('.compares-main');
  if (!comparesMain) return;
  if (favoritesBtns.length) {
    var supFavorites = document.querySelectorAll('#sup-favorites');
    favoritesBtns.forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        var glcFavCount = localCheng('glcFav', btn.dataset.postid);
        supFavorites.forEach(function (item) {
  ...
  if (comparisonBtns.length) {
    ...
    comparisonBtns.forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        var glcComparCount = localCheng('glcCompar', btn.dataset.postid);
        supComparison.forEach(function (item) {
...
function buttonsFavChange() {
  ...
  var supFavorites = document.querySelectorAll('#sup-favorites');
  var supСomparison = document.querySelectorAll('#sup-comparison');
  glampingsItems.addEventListener('click', function (event) {
    var btns = glampingsItems.querySelectorAll('button');
    ...
    if (btn) {
      ...
      if (btn.id == 'add-comparison') {
        var glcComparCount = localCheng('glcCompar', btn.dataset.postid);
        supСomparison.forEach(function (item) {
          item.innerHTML = glcComparCount;
        });
        btn.classList.toggle('active');
        if (btn.classList.contains('active')) {
...
function localCheng(name, value) {
  ...
  if (Cookies.get(name)) {
    ...
    var _ls = Cookies.get(name); // localStorage.getItem(name)
    ls_obj = _ls.split(',');
    if (ls_obj.includes(value)) {
      ls_obj = ls_obj.filter(function (i) {
        return i !== value;
      });
    } else {
      ls_obj.push(value);
...
function favoritesRender() {
  var supFavorites = document.querySelectorAll('#sup-favorites');
  var supComparison = document.querySelectorAll('#sup-comparison');
  var addFavorites = document.querySelectorAll('#add-favorites');
  ...
  if (supFavorites.length) {
    ...
    if (addFavorites.length) {
      addFavorites.forEach(function (item) {
  ...
  if (supComparison.length) {
    ...
    if (addComparison.length) {
      addComparison.forEach(function (item) {
...
function mapFavComAction(addFavorites, addFavoritesBtnSingle, supFavorites, btnType) {
  ...
  var glcFavCount = localCheng(btnType, addFavorites.dataset.postid);
  supFavorites.forEach(function (item) {
    item.innerHTML = glcFavCount;
  });
...
function paginateGlemp(glempAll) {
  var navLinks = document.querySelector('.pagination.filtr-pagination .nav-links');
  var perPage = glamping_club_ajax.glc_per_page.split('/').map(Number);
  // let pages = glempAll.length/perPage[0];
  var pages = glempAll.length / 1;
  var pagesCount = Math.ceil(pages);
  console.dir(perPage);
  console.dir(pages);
  ...
  for (var i = 0; i < pagesCount; i++) {
    if (i == 0) {
      ...
      pagContent += "<a class=\"page-numbers\" href=\"http://glamping".concat(window.location.pathname, "page/").concat(i + 1, "/\" datapage=\"").concat(i + 1, "\">").concat(i + 1, "</a>");
    }
  }
  pagContent += "<a class=\"next page-numbers\" href=\"http://glamping".concat(window.location.pathname, "page/2/\" datapage=\"2\">\n        <svg width=\"10\" height=\"14\" viewBox=\"0 0 10 14\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">\n        <path d=\"M1 13L8 7L1 1\" stroke=\"#5E6D77\" stroke-width=\"1.5\"></path>\n        </svg>\n    </a>");
  navLinks.innerHTML = pagContent;
}
// paginateGlemp(glempsAll);

function createPagination(pages, page) {
  var nav = document.querySelector('.pagination.filtr-pagination .nav-links');
  console.dir(pages);
  var str = '';
  var active;
  ...
  if (pages < 6) {
    ...
    for (var _p = pageCutLow; _p <= pageCutHigh; _p++) {
      ...
      active = page == _p ? "current" : "no";
      str += '<a class="page-numbers ' + active + '" onclick="createPagination(' + pages + ', ' + _p + ')">' + _p + '</a>';
    }
    if (page < pages - 1) {
      if (page < pages - 2) {
        str += '<a class="page-numbers" onclick="createPagination(' + pages + ',' + (page + 2) + ')">...</a>';
...
function navScroll(elementId) {
  ...
  var compareMobileTop = document.querySelector('.compare-wrap-mobile__top');
  var compareHeader = document.querySelector('.compares-main .entry-header');
  var compareSliderTop = document.querySelector('.compares-main .slider-top');
