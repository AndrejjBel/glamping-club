...
function chekAllFitrs() {
  ...
  var btnFiltrClear = document.querySelector('#all-filter-clear');
  var countObj = [];
...
function itemsChange() {
  ...
  filtrItems.addEventListener('click', function (event) {
    ...
    if (input) {
      var glempAll = JSON.parse(glamping_club_ajax.glAll);
      var newgGempAllPr = glempAll.filter(filtrOptionsChange);
      ...
      if (input.dataset.name != 'glcRegion') {
        if (localStorage.getItem('glcType') || localStorage.getItem('glcAllocation') || localStorage.getItem('glcWorking') || localStorage.getItem('glcNature') || localStorage.getItem('glcFacilGen') || localStorage.getItem('glcEntertainment') || localStorage.getItem('glcTerritory') || localStorage.getItem('glcSafety')) {
          locationsArchive(newgGempAll);
        } else {
          locationsArchive(glempAll);
        }
  ...
  glAll.forEach(function (item) {
    ...
    points.push({
      ...
      properties: {
        ...
        balloonContentBody: "<p class=\"ymaps-2-1-79-balloon-content__header\">\u043E\u0442 ".concat(item.price, "\u0440.</p> \u0410\u0434\u0440\u0435\u0441: ").concat(item.adress),
        balloonContentFooter: "<a href=\"".concat(item.url, "\">\u041F\u043E\u0434\u0440\u043E\u0431\u043D\u0435\u0435</a>"),
  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      ...
      if (btn.id == 'mapVision') {
        ...
        glampingsMap.children[0].innerHTML = '';
        var glempAll = JSON.parse(glamping_club_ajax.glAll);
        var _newgGempAll = glempAll.filter(filtrOptionsChange);
        var priceObj = [];
        priceObj = sliderUpdatePrice(_newgGempAll);
        _newgGempAll = glempAll.filter(filtrOptionsChange).filter(priceRange, priceObj);
        mapRender(mapPointTest(_newgGempAll));
