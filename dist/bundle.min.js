...
function sliderNumber(startMin, startMax, min, max) {
  ...
  slider.noUiSlider.on('end', function (values, handle) {
    ...
    var newgGempAll = glempsAll.filter(filtrOptionsChange).filter(priceRange, sliderValue);
...
function itemsChange() {
  ...
  filtrItems.addEventListener('click', function (event) {
    ...
    if (input) {
      ...
      var newgGempAllForFiltr = glempAll.filter(filtrOptionsChange);
      ...
      if (document.querySelector('input[name="stocks"]').checked) {
        newgGempAll = glempAll.filter(filtrOptionsChange).filter(priceRange, priceObj.map(Number)).filter(function (elem) {
          return elem.stocks != '';
        });
        newgGempAllForFiltr = glempAll.filter(filtrOptionsChangeForFiltr).filter(priceRange, priceObj.map(Number)).filter(function (elem) {
          return elem.stocks != '';
        });
      }
      if (document.querySelector('input[name="recommended"]').checked) {
  ...
  btnMap.forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      ...
      if (btn.id == 'mapVision') {
        ...
        glampingsMap.children[0].innerHTML = '';
        var newgGempAll = glempsAll.filter(filtrOptionsChange);
        var priceObj = [];
...
function sortGlempRender(sortGl) {
  ...
  // let glempAll = JSON.parse(glamping_club_ajax.glAll);
  var newgGempAll = glempsAll.filter(filtrOptionsChange).filter(priceRange, priceObj.map(Number));
...
function navScroll(elementId) {
  ...
  window.onscroll = function () {
    ...
    if (prevScrollpos > currentScrollPos) {
      if (masthead) {
        var _headHeight = masthead.offsetHeight + 20;
