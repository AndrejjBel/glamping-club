...
function itemsChange() {
  ...
  filtrItems.addEventListener('click', function (event) {
    ...
    if (input) {
      ...
      console.dir(sliderValueStart);
      var priceObj = [];
      var _glcPrice = localStorage.getItem('glcPrice');
      if (_glcPrice) {
        priceObj = _glcPrice.split(',');
        // sliderUpdatePriceNew(glcPrice);
      }
      ...
      if (input.dataset.name != 'glcRegion') {
        // locationsArchive(newgGempAll);
        if (glcType || glcAllocation || glcWorking || glcNature || glcFacilGen || glcEntertainment || glcTerritory || glcSafety && _glcPrice !== glcPriceSt) {
          locationsArchive(newgGempAll);
        } else if (glcType || glcAllocation || glcWorking || glcNature || glcFacilGen || glcEntertainment || glcTerritory || glcSafety && _glcPrice == glcPriceSt) {
          locationsArchive(newgGempAll);
        } else if (!glcType && !glcAllocation && !glcWorking && !glcNature && !glcFacilGen && !glcEntertainment && !glcTerritory && !glcSafety && _glcPrice !== glcPriceSt) {
          locationsArchive(newgGempAll);
        } else if (!glcType && !glcAllocation && !glcWorking && !glcNature && !glcFacilGen && !glcEntertainment && !glcTerritory && !glcSafety && _glcPrice == glcPriceSt) {
